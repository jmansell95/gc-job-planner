...
/**
 * Push a sign-out (asset assigned to a job) to Asset Panda — marks the
 * stock-status field as 'Out on Job' so the yard's Panda dashboard shows
 * where the gear is. Called by the Equipment Sign-Out flow after creating
 * local JobAssetAssignment records.
 *
 * @param base44      - base44 SDK client (asServiceRole)
 * @param pandaIds    - array of Asset Panda object IDs to mark as out on job
 * @param jobName     - optional job name to set in the status field
 * @returns { attempted, success, updated, errors }
 */
