...
function buildEmail(staff, rotas, jobs, vehicles, cfg, weekStart) {
  ...
  if (cfg && cfg.template) {
    ...
    const base = (cfg && cfg.intro_message) ? cfg.intro_message + '\n\n' : '';
    intro = base + 'Hi ' + staff.name + ', here is your schedule for the week of ' + weekLabel + '. You have ' + assignmentCount + ' assignment(s).';
  }
  ...
  const banner = showBanner
    ...
    : '';
  const introHtml = '<p style="font-size:14px;color:#475569;margin:0 0 16px 0;white-space:pre-wrap">' + escapeHtml(intro).replace(/\n/g, '<br>') + '</p>';
  const html = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;background:#f1f5f9;font-family:Arial,Helvetica,sans-serif">' +
    ...
    banner +
    '<tr><td style="padding:24px;color:#1e293b;font-size:14px;line-height:1.6">' + introHtml + table + '</td></tr>' +
...
Deno.serve(async (req) => {
  try {
    ...
    // Load the staff_schedule alert config
    const cfgList = await base44.asServiceRole.entities.EmailAlertSetting.filter({ alert_key: 'staff_schedule' });
    const cfg = cfgList[0] || { enabled: true, accent_color: '#0e7a4f', banner_title: 'GC Job Planner', show_banner: true, footer_text: 'GC Job Planner' };
    ...
    for (const s of staffList) {
      ...
      const { html, subject } = buildEmail(s, myRotas, jobs, vehicles, cfg, weekStart);
      try {
