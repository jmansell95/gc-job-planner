...
import { differenceInDays, differenceInMonths } from 'npm:date-fns@3.6.0';
...
const SERVICE_INTERVAL_MILES = 10000; // typical UK van service interval
const MOT_CYCLE_MONTHS = 12; // annual MOT
...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const vehicles = body.vehicle_id
      ? (await base44.asServiceRole.entities.Vehicle.filter({ id: body.vehicle_id }, '-created_date', 1))
      : await base44.asServiceRole.entities.Vehicle.list('-created_date', 500);

    // Recent location logs for mileage-rate calculation (last 30 days)
    ...
    // Maintenance bookings (for repair frequency)
    ...
    // Group MOT history by vehicle (sorted desc)
    ...
    for (const v of vehicles) {
      ...
      if (v.mot_expiry) {
        ...
        // Estimate next MOT = last test + 12 months
      ...
      if (v.service_due_date) {
        ...
        // Predict based on mileage rate: when will next service interval be hit?
      ...
      // Repair frequency (last 6 months)
      ...
      // Only include vehicles with enough data to predict
