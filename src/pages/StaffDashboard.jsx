    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-6">
          <h1 className="text-3xl font-bold text-green-600">My Schedule</h1>
          <p className="text-slate-600 mt-1">Welcome, {staff.name}</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-lg p-6 border border-green-200 shadow-sm mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-lg font-semibold text-slate-900 mt-1 capitalize">{staff.job_role.replace('_', ' ')}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase">Worker Type</p>
              <p className="text-lg font-semibold text-slate-900 mt-1 capitalize">{staff.worker_type.replace('_', ' ')}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase">Email</p>
              <p className="text-lg font-semibold text-slate-900 mt-1 text-sm">{staff.email}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 uppercase">Assigned Jobs</p>
              <p className="text-lg font-semibold text-slate-900 mt-1">{assignments.length}</p>
            </div>
          </div>
        </div>
        ...
        {/* Assignments List */}
        <div>
          <h2 className="text-xl font-bold text-slate-900 mb-4">This Week & Next Week</h2>
          ...
          {assignments.length === 0 ? (
            ...
            <div className="space-y-4">
              {assignments.map(assignment => {
                ...
                return (
                  <div key={assignment.id} className={`rounded-lg p-6 border-l-4 border ${jobTypeColors[job.job_type] || 'bg-slate-50 border-slate-200'}`}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="space-y-2 text-sm text-slate-600">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {format(new Date(assignment.assigned_date), 'EEEE, MMM d, yyyy')}
                          </div>
                          {job.client_id && (
                            <div className="flex items-center gap-2">
                      ...
                      <div>
                        {vehicle && (
                          <div className="mb-4 p-4 bg-white bg-opacity-50 rounded-lg border border-green-100">
                        ...
                        {job.equipment_needed && (
                          <div className="p-4 bg-white bg-opacity-50 rounded-lg border border-green-100">
                        ...
                        {job.notes && (
                          <div className="mt-4 p-4 bg-white bg-opacity-50 rounded-lg border border-green-100">
