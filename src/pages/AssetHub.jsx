...
import {
  ...
  HelpCircle, Plus, Search, Link2, ChevronRight, Boxes, Layers, ScanLine, X, TrendingUp,
  RefreshCw, Lock, Check, CheckSquare, Upload, Database, Wrench as WrenchIcon,
} from 'lucide-react';
...
import InventoryTab from '@/components/assethub/InventoryTab';
...
export default function AssetHub() {
  ...
  const [view, setView] = useState('rigs');
  ...
  const filteredEquip = equipment.filter(eq => {
    ...
    return true;
  }).map(eq => ({ equip: eq, parentRig: findParentRig(eq.id, rigs) }));

  const openAdd = () => { setEditorAsset(null); setEditorOpen(true); };
  const openEdit = (asset) => { setEditorAsset(asset); setEditorOpen(true); };

  const tabs = [
    { id: 'rigs', label: 'Rigs', icon: Boxes, count: rigs.length },
    { id: 'equipment', label: 'Equipment', icon: Layers, count: equipment.length },
  ...
  const headerStats = [
    ...
    { label: 'Rigs Need Attn', value: rigsWarning, icon: ShieldAlert },
    { label: 'Expired', value: equipExpired, icon: ShieldX },
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        icon={Wrench}
        title="Asset Hub"
        subtitle="Rigs, equipment, compliance, inventory & Asset Panda sync"
        stats={headerStats}
        actions={
          <>
            <button onClick={() => navigate('/pat-testing')} className="hidden sm:inline-flex items-center gap-1.5 px-3 py-2 bg-amber-400/90 hover:bg-amber-400 text-slate-900 rounded-lg font-semibold text-xs transition shadow-sm"><Plug className="w-3.5 h-3.5" /> PAT</button>
      ...
      {/* Fleet sync + health gauge + donut */}
      <div className="insight-card rounded-2xl p-4">
        <FleetSyncPanel />
        <div className="flex flex-col lg:flex-row gap-4 items-stretch mt-4">
          <div className="flex items-center justify-center gap-4 sm:gap-5 bg-slate-50 rounded-xl ring-1 ring-slate-200 px-4 sm:px-6 py-3 lg:flex-shrink-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 flex-1 w-full">
            {[
              ...
              { label: 'Rigs Need Attn', value: rigsWarning, icon: ShieldAlert, grad: 'stat-gradient-amber' },
              ...
              return (
                <Wrapper key={s.label} onClick={s.onClick} className={`${s.grad} rounded-xl p-3.5 text-left text-white shadow-lg ring-1 ring-white/20 ${s.onClick ? 'hover:brightness-110 active:scale-[0.98] transition cursor-pointer' : ''}`}>
      ...
      {isLoading ? (
        ...
        filteredRigs.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-10 text-center"><Boxes className="w-10 h-10 text-slate-200 mx-auto mb-2" /><p className="text-sm text-slate-400">{rigs.length === 0 ? 'Add a rig asset to get started.' : 'Try clearing your filters.'}</p></div>
        ) : (
                ...
                <button key={rig.id} onClick={() => setOpenRig(rig)} className={`insight-card rounded-xl p-4 text-left ${border}`}>
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0"><p className="font-semibold text-slate-900 truncate">{rig.name}</p>{rig.rig_type && rig.rig_type !== 'n/a' && <p className="text-[11px] text-blue-600 font-bold uppercase">{rig.rig_type} Rig</p>}{rig.serial_number && <p className="text-xs text-slate-400 font-mono truncate">{rig.serial_number}</p>}</div>
                    </div>
                  ...
                  <div className="flex gap-1.5">{['compliant', 'expiring', 'expired', 'unknown'].map(k => rollup.counts[k] > 0 && <span key={k} className={`flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full ${COMPLIANCE_META[k].tone}`}><span className={`w-1.5 h-1.5 rounded-full ${COMPLIANCE_META[k].dot}`} /> {rollup.counts[k]}</span>)}</div>
                  {(() => { const totalUnits = (rig.linked_equipment_ids || []).length + 1; const pct = totalUnits > 0 ? Math.round((rollup.counts.compliant / totalUnits) * 100) : 0; return (
                    <div className="mt-2"><div className="flex items-center justify-between text-[10px] text-slate-400 mb-1"><span>Compliance</span><span className="font-semibold text-slate-600">{pct}%</span></div><div className="h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${pct >= 85 ? 'bg-emerald-500' : pct >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${pct}%` }} /></div></div>
                  ); })()}
                  ...
                  <span role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); setCertVaultRig(rig); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); setCertVaultRig(rig); } }} className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-[#2E5A1A]/10 hover:bg-[#2E5A1A]/20 text-[#2E5A1A] rounded-lg text-[11px] font-semibold transition w-full justify-center"><Lock className="w-3.5 h-3.5" /> View Certificates</span>
                </button>
              );
            })}
          </div>
        )
      ) : view === 'equipment' && filteredEquip.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-10 text-center"><Layers className="w-10 h-10 text-slate-200 mx-auto mb-2" /><p className="text-sm text-slate-400">{equipment.length === 0 ? 'Add equipment assets to get started.' : 'Try clearing your filters.'}</p></div>
      ) : view === 'equipment' ? (
        <>
          <div className="mb-4 flex items-center justify-between gap-3 flex-wrap">
            <EquipmentFilters assets={equipment} filters={equipFilters} setFilters={setEquipFilters} />
            <button onClick={() => { setSelectionMode(m => !m); setSelected(new Set()); }} className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold transition flex-shrink-0 ${selectionMode ? 'bg-emerald-600 text-white shadow-sm' : 'bg-white text-slate-600 border border-slate-300 hover:bg-slate-50'}`}><CheckSquare className="w-4 h-4" /> {selectionMode ? 'Done' : 'Select'}</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filteredEquip.map(({ equip, parentRig }) => {
              ...
              const handleCardClick = () => { if (selectionMode) { setSelected(prev => { const n = new Set(prev); n.has(equip.id) ? n.delete(equip.id) : n.add(equip.id); return n; }); } else { setOpenEquip(equip); } };
              return (
                <div key={equip.id} onClick={handleCardClick} className={`insight-card rounded-xl p-4 text-left relative ${statusAccent} ${selectionMode ? 'cursor-pointer' : ''} ${isSel ? 'ring-2 ring-emerald-500' : ''}`}>
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2.5">
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0"><p className="font-semibold text-slate-900 truncate">{equip.name}</p><p className="text-[11px] text-slate-400 truncate">{ASSET_TYPE_META[equip.asset_type]?.label || equip.asset_type}{equip.equipment_type ? ` · ${equip.equipment_type}` : ''}</p></div>
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {parentRig ? <span className="text-[10px] text-emerald-700 font-medium flex items-center gap-0.5"><Link2 className="w-3 h-3" /> {parentRig.name}</span> : <span className="text-[10px] text-slate-400">Unlinked</span>}
                  </div>
      ...
      {/* Drawers & modals */}
      {openRig && <RigDetailDrawer rig={openRig} allAssets={assets} onClose={() => setOpenRig(null)} onOpenEquipment={(eq) => setOpenEquip(eq)} onEdit={(a) => { setOpenRig(null); openEdit(a); }} onRecert={(a) => { setOpenRig(null); setRecertAsset(a); }} />}
      {openEquip && <EquipmentDetailDrawer equipment={openEquip} parentRig={findParentRig(openEquip.id, rigs)} onClose={() => setOpenEquip(null)} onOpenRig={(rig) => { setOpenEquip(null); setOpenRig(rig); }} onEdit={(a) => { setOpenEquip(null); openEdit(a); }} onRecert={(a) => { setOpenEquip(null); setRecertAsset(a); }} />}
      ...
      {selectionMode && filteredEquip.length > 0 && <BulkActionBar count={selected.size} total={filteredEquip.length} onClear={() => { setSelected(new Set()); setSelectionMode(false); }} onSelectAll={() => setSelected(new Set(filteredEquip.map(({ equip }) => equip.id)))} onViewCerts={() => setBulkCerts(filteredEquip.filter(({ equip }) => selected.has(equip.id)).map(({ equip }) => equip))} />}
      {bulkCerts && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 pt-8 sm:pt-4">
