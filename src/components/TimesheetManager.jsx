              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200">
                    <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Staff</th>
                ...
                return (
                  <div key={ts.id} className="p-4">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center flex-shrink-0">
                          <span className="text-emerald-700 font-bold text-[11px]">{(member?.name || '?').charAt(0)}</span>
                        </div>
                        <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-2">
                      <CalendarDays className="w-3.5 h-3.5 text-slate-400" />
                      <span>{format(new Date(ts.date + 'T00:00:00'), 'EEE dd MMM yyyy')}</span>
                    </div>
                    <p className="text-sm text-slate-700 mb-1.5">{ts.task_description || <span className="text-slate-400 italic">{ts.start_time ? `${ts.start_time}–${ts.end_time}` : '—'}</span>}</p>
                    {ts.notes && <p className="text-xs text-slate-400 truncate mb-1.5">· {ts.notes}</p>}
                    {ts.status === 'deleted' && ts.deletion_reason && <p className="text-xs text-red-500 truncate mb-1.5">⊘ {ts.deletion_reason}</p>}
                    <div className="flex items-center gap-3 flex-wrap text-xs mb-1">
                      <span className="inline-flex items-center gap-1 font-semibold text-slate-900">
                        <Clock className="w-3 h-3 text-slate-400" />{mtr > 0 ? `${mtr}m` : fmtMins(mins)}
                      </span>
                      {b.isOvertime && (
                    ...
                    {ts.status === 'submitted' && (
                      <div className="flex gap-2 mt-3">
                        <button onClick={() => handleApprove(ts.id)} className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 text-white rounded-lg text-xs font-semibold active:scale-95 transition">
