...
import {
  Package, Calendar, HardHat, User, Lock, Receipt, Boxes, Building2, Upload, FileText, Loader2, X
} from 'lucide-react';
...
export default function EquipmentForm({ form, setForm, onSubmit, onCancel, saving = false, editing = false, suppliers = [], contractors = [], defaultDates = null, catalogueItems = [], rateCardItems = [], ownedAssets = [] }) {
  ...
  // Rate card items: scope by category and supplier
  ...
  return (
    <div onKeyDown={handleKeyDown} className="border border-emerald-200 rounded-lg p-4 space-y-3 bg-emerald-50/30">
        ...
        {isContractorSupplied ? (
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">
              ...
              {!isInternal && !isPurchased && <span className="text-slate-400 font-normal"> (optional)</span>}
            </label>
            <select value={form.supplier_id} onChange={(e) => setForm({ ...form, supplier_id: e.target.value, rate_card_item_id: '' })} className={inputCls} disabled={isInternal}>
              <option value="">{isInternal ? 'N/A' : 'Select supplier…'}</option>
      ...
      <div className="flex gap-2 pt-1">
        <button type="button" onClick={submit} disabled={saving || uploadingOrderSlip || !form.description?.trim() || (!isNoCost && !form.unit_cost) || (isPurchased && !form.po_number?.trim())} className="px-4 py-2 bg-emerald-700 text-white rounded-lg text-sm font-medium hover:bg-emerald-800 transition disabled:opacity-50 flex items-center gap-2">
