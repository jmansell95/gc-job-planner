...
export default function EquipmentForm({ form, setForm, onSubmit, onCancel, saving = false, editing = false, suppliers = [], defaultDates = null }) {
  const daysFromForm = () => {
  ...
  const submit = () => {
    ...
    onSubmit({ ...form, quantity: effectiveQty });
  };
  ...
  return (
    <div onKeyDown={handleKeyDown} className="border border-emerald-200 rounded-lg p-4 space-y-3 bg-emerald-50/30">
      ...
      <div className="flex gap-2 pt-1">
        <button type="button" onClick={submit} disabled={saving || !form.description?.trim() || !form.unit_cost} className="px-4 py-2 bg-emerald-700 text-white rounded-lg text-sm font-medium hover:bg-emerald-800 transition disabled:opacity-50">
