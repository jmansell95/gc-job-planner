    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="border-b border-slate-100 pb-4 space-y-3">
          <p className="text-xs font-semibold text-slate-700">Web Login (recommended) — the email + password you sign into app.assetpanda.com with</p>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Email <span className="text-red-500">*</span></label>
            <input type={showSecrets ? 'text' : 'text'} value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className={inputCls} placeholder="you@company.com" autoComplete="username" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Password <span className="text-red-500">*</span></label>
            <input type={showSecrets ? 'text' : 'password'} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} className={inputCls} placeholder="Your Asset Panda web password" autoComplete="current-password" />
          </div>
          <p className="text-[11px] text-slate-400">The app uses these to generate a fresh session token on every sync, so you never hit an expired-token error. Do <strong>not</strong> use the Client ID/Client Secret from the API Configuration page — that UUID is rejected by the session endpoint.</p>
        </div>

        <div className="space-y-2">
          <p className="text-xs font-semibold text-slate-500">— or paste a pre-generated session token (advanced) —</p>
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">Session tokens expire — prefer using the web login email + password above.</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
          ...
          <button onClick={handleTest} disabled={testing || !(form.email || form.api_token)}
