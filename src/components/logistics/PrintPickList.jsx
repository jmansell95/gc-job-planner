...
/**
 * Printable per-drop pick list for the warehouse team. Opens a
 * print-optimised window showing every detail the warehouse needs to pick,
 * stage and load a single delivery stop: pickup point, destination, driver,
 * vehicle, contact, PO number, items with tick-off columns, notes and a
 * loading-bay sign-off line. Uses the same window.open + print pattern as
 * PrintLoadManifest.
 *
 * Props:
 *   delivery  — the DeliveryLog record for this drop
 *   job       — optional Job record (for reference / what3words / site contact)
 *   vehicle   — optional Vehicle record (reg + height for loading bay)
 *   driverName— driver display name
 */
export default function PrintPickList({ delivery, job, vehicle, driverName }) {
  const handlePrint = () => {
    ...
    // Parse the free-text items list into individual pick lines
    const rawItems = (delivery?.items || '')
      .split(/\n|,(?=\s)/)
      .map(x => x.trim())
      .filter(Boolean);

    const itemRows = (rawItems.length ? rawItems : ['(no items listed)']).map((line, i) => {
      return `<tr>
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Pick List — ${delivery?.job_name || 'Drop'}</title>
    <style>
      @page { margin: 1.2cm; }
  ...
  return (
    <button onClick={handlePrint} type="button"
      ...
      <ClipboardList className="w-3.5 h-3.5" /> Pick List
    </button>
  );
}