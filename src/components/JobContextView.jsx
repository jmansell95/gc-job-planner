...
/**
 * JobContextView — the high-density multi-pane "Context" view.
 * Surfaces job vitals, live financials, and recent activity simultaneously
 * in a responsive three-pane layout (collapses to stacked on mobile).
 * Designed for managers who need everything at a glance without tab-switching.
 */
export default function JobContextView({ job, primaryType, assignedStaff, rotas, client, contractor, suppliers, canSeeCosts, isDrillingJob, colors, statusBadge: sb, statusLabels: sl, startDate, endDate, jobProject, jobTypes }) {
  ...
  const dayCount = startDate && endDate ? Math.max(1, Math.round((endDate - startDate) / (1000 * 60 * 60 * 24)) + 1) : 0;

  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
      {/* LEFT PANE: Job Vitals */}
      <div className="lg:col-span-3 space-y-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className={'px-4 py-3 ' + colors.bg + ' ' + colors.border + ' border-b'}>
            <div className="flex items-center gap-2 mb-1">
              <span className={'inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-white/80 ' + colors.text}>
                <span className={'w-1.5 h-1.5 rounded-full ' + colors.dot} />
                {getJobTypeLabel(primaryType, jobTypes)}
              </span>
              <span className={'inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ' + sb[job.status || 'planning']}>
                {sl[job.status || 'planning']}
              </span>
            </div>
            <h2 className="text-base font-bold text-slate-900 leading-tight truncate">{job.name}</h2>
          </div>
          <div className="px-4 py-2 divide-y divide-slate-50">
            <VitalsField label="Location" icon={MapPin}>{job.location || '—'}</VitalsField>
            {job.job_reference && <VitalsField label="Reference" icon={FileText}>{job.job_reference}</VitalsField>}
            <VitalsField label="Duration" icon={Calendar}>
              {startDate ? format(startDate, 'dd MMM') + ' → ' + (endDate ? format(endDate, 'dd MMM') : 'TBC') : '—'}
            ...
            {job.budget_amount != null && canSeeCosts && <VitalsField label="Budget" icon={PoundSterling}>{fmt2(job.budget_amount)}</VitalsField>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3">
          ...
          {assignedStaff.length === 0 ? (
            <p className="text-xs text-slate-400 py-2">No crew assigned yet</p>
          ) : (
