...
export default function SafetyCultureCheckHub({ onNavigate }) {
  ...
  const stats = [
    { icon: AlertTriangle, label: 'Overdue Actions', value: overdueActions.length, sub: 'Past their due date', tone: overdueActions.length > 0 ? 'rose' : 'emerald' },
    { icon: ClipboardCheck, label: 'Audits This Week', value: auditsThisWeek, sub: 'Completed in last 7 days', tone: 'blue' },
    { icon: Users, label: 'People Completing Checks', value: byAuditor.length, sub: 'Unique auditors', tone: 'amber' },
    { icon: ShieldAlert, label: 'Open Audits', value: reports.filter((r) => r.status === 'open').length, sub: 'Awaiting close-out', tone: 'slate' },
  ];
  ...
  return (
    <div>
      <PageHeader
        title="SafetyCulture Check Completion"
        icon={ShieldAlert}
        subtitle="Who has completed daily vehicle, plant & equipment and POWRA checks — plus overdue safety actions"
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        {stats.map((s) => {
          ...
          return (
            <div key={s.label} className="insight-card rounded-2xl p-4 relative overflow-hidden">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ring-1 ${t}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <p className="text-2xl font-bold tabular-nums leading-none">{s.value}</p>
                  <p className="text-xs font-semibold text-slate-600 mt-1 truncate">{s.label}</p>
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-slate-400">
        ...
        Audits sync automatically from SafetyCulture (iAuditor) via webhook. Configure the connection in Settings → SafetyCulture.
      </div>
    </div>
  );
}