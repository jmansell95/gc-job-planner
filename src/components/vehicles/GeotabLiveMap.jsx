...
import { MapPin, Satellite, Loader2, RefreshCw, Navigation, Gauge, Clock, Car, FileBarChart, X } from 'lucide-react';
...
function VehicleMarker({ vehicle, onClick }) {
  ...
  const icon = window.L?.divIcon({
    html: `<div style="background:${vehicle.ignition_on ? '#2E5A1A' : '#94a3b8'};width:28px;height:28px;border-radius:50%;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center">
    ...
    <Marker position={pos} icon={icon} eventHandlers={{ click: () => onClick(vehicle) }}>
      <Popup>
        <div className="text-xs space-y-1">
          <p className="font-bold text-sm">{vehicle.registration_number}</p>
...
export default function GeotabLiveMap() {
  ...
  const trackedCount = vehicles.length;
  const movingCount = vehicles.filter(v => v.ignition_on).length;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-3 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 flex-1 min-w-[200px]">
          <Satellite className="w-5 h-5 text-[#2E5A1A]" />
          <div>
            <p className="text-sm font-bold text-slate-800">Live Fleet Map</p>
            <p className="text-[11px] text-slate-500">{trackedCount} vehicles tracked · {movingCount} moving now</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div style={{ height: '500px' }} className="rounded-xl">
              <MapContainer center={UK_CENTER} zoom={6} style={{ height: '100%', width: '100%' }} scrollWheelZoom>
                ...
                {historyPath.length > 1 && (
                  <Polyline positions={historyPath} pathOptions={{ color: '#2E5A1A', weight: 3, opacity: 0.6 }} />
                )}
              </MapContainer>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5"><Car className="w-4 h-4 text-[#2E5A1A]" /> Vehicles ({trackedCount})</p>
            </div>
            <div className="max-h-[448px] overflow-y-auto divide-y divide-slate-50">
              {vehicles.map(v => (
                <button key={v.vehicle_id || v.registration_number}
                  onClick={() => setSelectedVehicle(v)}
                  className={`w-full text-left px-4 py-3 hover:bg-slate-50 transition ${selectedVehicle?.vehicle_id === v.vehicle_id ? 'bg-emerald-50' : ''}`}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${v.ignition_on ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  </div>
      ...
      {selectedVehicle && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-4 pt-8 sm:pt-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between gap-3 rounded-t-2xl">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900">{selectedVehicle.registration_number}</h3>
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 rounded-lg p-3">
                  ...
                  <p className="text-sm font-bold flex items-center gap-1.5">
                    <span className={`w-2 h-2 rounded-full ${selectedVehicle.ignition_on ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                    {selectedVehicle.ignition_on ? 'Engine On' : 'Engine Off'}
                  </p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
                  <p className="text-[10px] uppercase text-slate-400 font-semibold">Speed</p>
                  <p className="text-sm font-bold">{selectedVehicle.speed_kph} km/h</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
                  <p className="text-[10px] uppercase text-slate-400 font-semibold">Coordinates</p>
                  <p className="text-xs font-mono">{selectedVehicle.lat.toFixed(4)}, {selectedVehicle.lng.toFixed(4)}</p>
                </div>
                <div className="bg-slate-50 rounded-lg p-3">
                  <p className="text-[10px] uppercase text-slate-400 font-semibold">Last Seen</p>
                  <p className="text-xs">{selectedVehicle.timestamp ? new Date(selectedVehicle.timestamp).toLocaleString('en-GB') : '—'}</p>
                </div>
              </div>
              ...
              {history && history.count > 0 && (
                <div>
                  <p className="text-xs font-bold text-slate-700 mb-2">Location History ({history.count} points)</p>
