        ...
        {scopedJobs.map(job => {
          ...
          return (
            ...
            >
              ...
              {/* Crew + activity footer */}
              <div className="flex items-center justify-between gap-2 pl-1.5 pt-2 border-t border-slate-100">
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-1.5">
                    {crewToday.slice(0, 3).map(m => (
                      <div key={m.id} className="w-6 h-6 rounded-full bg-gradient-to-br from-[#2E5A1A] to-[#5A8C1E] ring-2 ring-white flex items-center justify-center">
                    ...
                    {crewToday.length > 3 && (
                      <div className="w-6 h-6 rounded-full bg-slate-200 ring-2 ring-white flex items-center justify-center">
                        <span className="text-slate-600 font-bold text-[9px]">+{crewToday.length - 3}</span>
                      </div>
                    )}
                  </div>
                  <span className="text-xs text-slate-500">
                    {crewToday.length > 0 ? `${crewToday.length} on site` : 'No crew today'}
                  </span>
                </div>
                {activityCount > 0 && (
                  <span className="inline-flex items-center gap-1 text-xs text-violet-600 font-medium">
                    <Activity className="w-3 h-3" /> {activityCount}
                  </span>
                )}
              </div>
