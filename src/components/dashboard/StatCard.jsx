...
export default function StatCard({
  ...
  return (
    <Tag
      ...
      onClick={onClick}
      className={`relative rounded-2xl border p-4 bg-gradient-to-br from-white to-slate-50/50 flex items-center gap-3.5 text-left transition-all duration-200 ${active ? 'border-emerald-400 ring-2 ring-emerald-100 shadow-md' : interactive ? 'border-slate-200 hover:border-emerald-300 hover:shadow-md hover:-translate-y-0.5 group' : 'border-slate-200 shadow-sm'} ${interactive ? 'group' : ''} ${className}`}
    >
      <div className={`relative w-12 h-12 rounded-xl ${gradient} flex items-center justify-center flex-shrink-0 shadow-md icon-tile-glow`}>
        <Icon className="w-6 h-6 text-white" />
      </div>
      <div className="min-w-0 flex-1">
        <p className={`text-xl font-extrabold text-slate-900 truncate tabular-nums ${valueClassName}`}>{value}</p>
        <p className="text-[11px] text-slate-500 font-semibold truncate uppercase tracking-wide">{label}</p>
