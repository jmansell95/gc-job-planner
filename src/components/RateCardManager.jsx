...
import { PoundSterling, Search, Plus, Pencil, Check, X, Users, Wrench, Package, Loader2 } from 'lucide-react';
      ...
      {editing ? (
        <div className="p-3 bg-slate-50 space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <div>
              ...
              <input value={form.price_text} onChange={e => setForm({ ...form, price_text: e.target.value })}
                className={inputCls} placeholder="POA" />
            </div>
            <div>
              <label className="text-[10px] text-slate-400 font-medium">Unit</label>
              <input value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })}
                className={inputCls} placeholder="day" />
            </div>
            <div>
              <label className="text-[10px] text-slate-400 font-medium">Men</label>
              <input type="number" value={form.men} onChange={e => setForm({ ...form, men: e.target.value })}
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={saving}
              className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-700 text-white rounded-lg text-xs font-semibold hover:bg-emerald-800 disabled:opacity-50">
              {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />} Save
            </button>
            <button onClick={() => setEditing(false)}
              className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-200 text-slate-600 rounded-lg text-xs font-semibold hover:bg-slate-300">
        ...
        <div className="flex items-start gap-3 px-4 py-2.5 hover:bg-slate-50/50 transition group">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <span className={`text-sm font-semibold tabular-nums w-20 text-right ${item.price != null ? 'text-slate-900' : 'text-slate-400 italic'}`}>
              {priceDisplay}
            </span>
            <button onClick={() => { setForm({ description: item.description, price: item.price ?? '', price_text: item.price_text ?? '', unit: item.unit ?? '', men: item.men ?? '', notes: item.notes ?? '' }); setEditing(true); }}
              className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-emerald-700 transition">
...
function AddRateForm({ category, subcategory, onAdded }) {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)}
        className="w-full flex items-center justify-center gap-1.5 py-2 text-xs text-slate-400 hover:text-emerald-700 hover:bg-emerald-50/50 rounded-lg transition border border-dashed border-slate-200">
  ...
  return (
    <div className="p-3 bg-slate-50 border-b border-slate-100 space-y-2">
      <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
        placeholder="Description" className={inputCls} autoFocus />
      <div className="grid grid-cols-2 gap-2">
        <input type="number" step="0.01" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })}
          placeholder="Price £" className={inputCls} />
        <input value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })}
          placeholder="Unit (day, hour, m)" className={inputCls} />
      </div>
      <input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
        placeholder="Notes" className={inputCls} />
      <div className="flex gap-2">
...
export default function RateCardManager() {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['rate-card-items'] });
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2 flex-wrap">
        ...
        <h2 className="font-semibold text-slate-900">Schedule of Rates 2026</h2>
        <span className="ml-auto text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-medium">{items.length} rates</span>
      </div>
        ...
        {Object.entries(CATEGORY_META).map(([key, meta]) => {
          ...
          return (
            <button key={key} onClick={() => setActiveCategory(key)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-t-lg text-sm font-medium transition border-b-2 ${
                active ? 'border-emerald-600 text-emerald-700 bg-emerald-50/50' : 'border-transparent text-slate-500 hover:text-slate-700'
              }`}>
      ...
      <div className="px-4 py-3 border-b border-slate-100">
        <div className="relative">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder={`Search ${CATEGORY_META[activeCategory].label.toLowerCase()} rates...`}
      ...
      <div className="overflow-y-auto max-h-[60vh]">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-sm text-slate-400">No rates found.</div>
        ) : (
          grouped.map(([subcategory, subItems]) => (
            <div key={subcategory} className="border-b border-slate-100 last:border-0">
              ...
              {subItems.map(item => (
                <RateItemRow key={item.id} item={item} onUpdate={refresh} />
              ))}
              <AddRateForm category={activeCategory} subcategory={subcategory} onAdded={refresh} />
            </div>
          ))
        )}
      </div>
